...
export default function TasksView({ tasks, setTasks, goals, sessions, setSessions, onStartFocus, onNavigate, isGuest }: TasksViewProps) {
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h2 className="text-5xl font-black tracking-tighter text-brand-text flex items-center gap-6">
            <div className="w-16 h-16 rounded-[2rem] bg-brand-blue/10 flex items-center justify-center shadow-inner">
              <ListTodo className="w-8 h-8 text-brand-blue" />
            </div>
            Daily Missions
          </h2>
          <p className="text-brand-text-light font-medium mt-4 px-2 text-lg">Execute with precision. Conquer your objectives.</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="bg-white border border-brand-primary/10 rounded-3xl p-1.5 flex items-center gap-1 shadow-xl">
             <button 
                ...
                className={cn(
                  "flex flex-col items-center px-6 py-2 rounded-2xl transition-all",
                  statusFilter === 'pending' ? "bg-brand-orange/10 ring-1 ring-brand-orange/20" : "hover:bg-brand-bg"
                )}
             >
                <span className="text-[10px] font-black text-brand-text-light uppercase tracking-widest mb-1">Pending</span>
                <span className="text-2xl font-black text-brand-orange">{pendingTasks}</span>
             </button>
             ...
             >
                <span className="text-[10px] font-black text-brand-text-light uppercase tracking-widest mb-1">Success</span>
                <span className="text-2xl font-black text-brand-green">{completionRate}%</span>
             </button>
          </div>
          <motion.button 
      ...
      {/* Tasks List */}
      <section className="bg-white border border-brand-primary/10 rounded-[3rem] overflow-hidden shadow-2xl">
          ...
          {filteredTasks.map((task, idx) => {
            ...
            return (
              <motion.div
                ...
                className={cn(
                  "p-8 flex items-center gap-8 transition-all group relative",
              ...
              >
                ...
                <button 
                  ...
                  className={cn(
                    "w-12 h-12 rounded-2xl flex items-center justify-center transition-all shadow-lg active:scale-90 shrink-0",
                ...
                >
                  <CheckCircle2 className={cn("w-6 h-6", task.completed ? "opacity-100" : "opacity-0 group-hover:opacity-100")} />
                </button>

                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-4 mb-2">
                    <h4 className={cn(
                      "text-2xl font-black tracking-tight transition-all truncate",
                ...
                <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                  <button 
                    ...
                    className="p-4 bg-brand-bg hover:bg-white rounded-[1.5rem] text-brand-text-light hover:text-brand-primary transition-all shadow-sm border border-brand-primary/5"
                  >
                    <Pencil className="w-5 h-5" />
                  </button>
                  <button 
                    onClick={() => handleDeleteTask(task.id)}
                    className="p-4 bg-brand-bg hover:bg-white rounded-[1.5rem] text-brand-red transition-all shadow-sm border border-brand-primary/5"
                  >
                  ...
                  {!task.completed && (
                    <button 
                      ...
                      className="p-4 bg-brand-primary text-white rounded-[1.5rem] shadow-xl shadow-brand-primary/20 hover:scale-110 active:scale-95 transition-all"
                    >
